{
  right = {
    "@jest/schemas@28.0.2" = {
      dev = true;
      integrity =
        "sha512-YVDJZjd4izeTDkij00vHHAymNXQ6WWsdChFRK86qck6Jpr3DCL5W3Is3vslviRlP+bLuMYRLbdp98amMvqudhA==";
      key = "@jest/schemas@28.0.2";
      name = "@jest/schemas";
      requires = { "@sinclair/typebox" = "^0.23.3"; };
      resolved =
        "https://registry.npmjs.org/@jest/schemas/-/schemas-28.0.2.tgz";
      version = "28.0.2";
    };
    "@sinclair/typebox@0.23.5" = {
      dev = true;
      integrity =
        "sha512-AFBVi/iT4g20DHoujvMH1aEDn8fGJh4xsRGCP6d8RpLPMqsNPvW01Jcn0QysXTsg++/xj25NmJsGyH9xug/wKg==";
      key = "@sinclair/typebox@0.23.5";
      name = "@sinclair/typebox";
      resolved =
        "https://registry.npmjs.org/@sinclair/typebox/-/typebox-0.23.5.tgz";
      version = "0.23.5";
    };
    "@types/jest-diff@24.3.0" = {
      dev = true;
      integrity =
        "sha512-vx1CRDeDUwQ0Pc7v+hS61O1ETA81kD04IMEC0hS1kPyVtHDdZrokAvpF7MT9VI/fVSzicelUZNCepDvhRV1PeA==";
      key = "@types/jest-diff@24.3.0";
      name = "@types/jest-diff";
      requires = { jest-diff = "*"; };
      resolved =
        "https://registry.npmjs.org/@types/jest-diff/-/jest-diff-24.3.0.tgz";
      version = "24.3.0";
    };
    "@types/jest@24.0.15" = {
      dev = true;
      integrity =
        "sha512-MU1HIvWUme74stAoc3mgAi+aMlgKOudgEvQDIm1v4RkrDudBh1T+NFp5sftpBAdXdx1J0PbdpJ+M2EsSOi1djA==";
      key = "@types/jest@24.0.15";
      name = "@types/jest";
      requires = { "@types/jest-diff" = "*"; };
      resolved = "https://registry.npmjs.org/@types/jest/-/jest-24.0.15.tgz";
      version = "24.0.15";
    };
    "@types/minimatch@3.0.3" = {
      dev = true;
      integrity =
        "sha512-tHq6qdbT9U1IRSGf14CL0pUlULksvY9OZ+5eEgl1N7t+OA3tGvNpxJCzuKQlsNgCVwbAs670L1vcVQi8j9HjnA==";
      key = "@types/minimatch@3.0.3";
      name = "@types/minimatch";
      resolved =
        "https://registry.npmjs.org/@types/minimatch/-/minimatch-3.0.3.tgz";
      version = "3.0.3";
    };
    "@types/node@12.20.55" = {
      dev = true;
      integrity =
        "sha512-J8xLz7q2OFulZ2cyGTLE1TbbZcjpno7FaN6zdJNrgAdrJ+DZzh/uFR6YrTb4C+nXakvud8Q4+rbhoIWlYQbUFQ==";
      key = "@types/node@12.20.55";
      name = "@types/node";
      resolved = "https://registry.npmjs.org/@types/node/-/node-12.20.55.tgz";
      version = "12.20.55";
    };
    "@types/zeromq@4.6.3" = {
      dev = true;
      integrity =
        "sha512-nuCaHBIvIi94LJygkHlLXv7dDK9ocNEKvOoa16n/XxXJhQHf5JX8Hd0Aq4fqhmY+2rAcBfsAFmQfqWWMuB7qUQ==";
      key = "@types/zeromq@4.6.3";
      name = "@types/zeromq";
      requires = { "@types/node" = "*"; };
      resolved = "https://registry.npmjs.org/@types/zeromq/-/zeromq-4.6.3.tgz";
      version = "4.6.3";
    };
    "ansi-regex@5.0.1" = {
      dev = true;
      integrity =
        "sha512-quJQXlTSUGL2LH9SUXo8VwsY4soanhgo6LNSm84E1LBcE8s3O0wpdiRzyR9z/ZZJMlMWv37qOOb9pdJlMUEKFQ==";
      key = "ansi-regex@5.0.1";
      name = "ansi-regex";
      resolved = "https://registry.npmjs.org/ansi-regex/-/ansi-regex-5.0.1.tgz";
      version = "5.0.1";
    };
    "ansi-styles@4.3.0" = {
      dev = true;
      integrity =
        "sha512-zbB9rCJAT1rbjiVDb2hqKFHNYLxgtk8NURxZ3IZwD3F6NtxbXZQCnnSi1Lkx+IDohdPlFp222wVALIheZJQSEg==";
      key = "ansi-styles@4.3.0";
      name = "ansi-styles";
      requires = { color-convert = "^2.0.1"; };
      resolved =
        "https://registry.npmjs.org/ansi-styles/-/ansi-styles-4.3.0.tgz";
      version = "4.3.0";
    };
    "ansi-styles@5.2.0" = {
      dev = true;
      integrity =
        "sha512-Cxwpt2SfTzTtXcfOlzGEee8O+c+MmUgGrNiBcXnuWxuFJHe6a5Hz7qwhwe5OgaSYI0IJvkLqWX1ASG+cJOkEiA==";
      key = "ansi-styles@5.2.0";
      name = "ansi-styles";
      resolved =
        "https://registry.npmjs.org/ansi-styles/-/ansi-styles-5.2.0.tgz";
      version = "5.2.0";
    };
    "backbone@1.4.1" = {
      dev = true;
      integrity =
        "sha512-ADy1ztN074YkWbHi8ojJVFe3vAanO/lrzMGZWUClIP7oDD/Pjy2vrASraUP+2EVCfIiTtCW4FChVow01XneivA==";
      key = "backbone@1.4.1";
      name = "backbone";
      requires = { underscore = ">=1.8.3"; };
      resolved = "https://registry.npmjs.org/backbone/-/backbone-1.4.1.tgz";
      version = "1.4.1";
    };
    "balanced-match@1.0.2" = {
      dev = true;
      integrity =
        "sha512-3oSeUO0TMV67hN1AmbXsK4yaqU7tjiHlbxRDZOpH0KW9+CeX4bRAaX0Anxt0tx2MrpRpWwQaPwIlISEJhYU5Pw==";
      key = "balanced-match@1.0.2";
      name = "balanced-match";
      resolved =
        "https://registry.npmjs.org/balanced-match/-/balanced-match-1.0.2.tgz";
      version = "1.0.2";
    };
    "brace-expansion@1.1.11" = {
      dev = true;
      integrity =
        "sha512-iCuPHDFgrHX7H2vEI/5xpz07zSHB00TpugqhmYtVmMO6518mCuRMoOYFldEBl0g187ufozdaHgWKcYFb61qGiA==";
      key = "brace-expansion@1.1.11";
      name = "brace-expansion";
      requires = {
        balanced-match = "^1.0.0";
        concat-map = "0.0.1";
      };
      resolved =
        "https://registry.npmjs.org/brace-expansion/-/brace-expansion-1.1.11.tgz";
      version = "1.1.11";
    };
    "chalk@4.1.2" = {
      dev = true;
      integrity =
        "sha512-oKnbhFyRIXpUuez8iBMmyEa4nbj4IOQyuhc/wy9kY7/WVPcwIO9VA668Pu8RkO7+0G76SLROeyw9CpQ061i4mA==";
      key = "chalk@4.1.2";
      name = "chalk";
      requires = {
        ansi-styles = "^4.1.0";
        supports-color = "^7.1.0";
      };
      resolved = "https://registry.npmjs.org/chalk/-/chalk-4.1.2.tgz";
      version = "4.1.2";
    };
    "color-convert@2.0.1" = {
      dev = true;
      integrity =
        "sha512-RRECPsj7iu/xb5oKYcsFHSppFNnsj/52OVTRKb4zP5onXwVF3zVmmToNcOfGC+CRDpfK/U584fMg38ZHCaElKQ==";
      key = "color-convert@2.0.1";
      name = "color-convert";
      requires = { color-name = "~1.1.4"; };
      resolved =
        "https://registry.npmjs.org/color-convert/-/color-convert-2.0.1.tgz";
      version = "2.0.1";
    };
    "color-name@1.1.4" = {
      dev = true;
      integrity =
        "sha512-dOy+3AuW3a2wNbZHIuMZpTcgjGuLU/uBL/ubcZF9OXbDo8ff4O8yVp5Bf0efS8uEoYo5q4Fx7dY9OgQGXgAsQA==";
      key = "color-name@1.1.4";
      name = "color-name";
      resolved = "https://registry.npmjs.org/color-name/-/color-name-1.1.4.tgz";
      version = "1.1.4";
    };
    "concat-map@0.0.1" = {
      dev = true;
      integrity =
        "sha512-/Srv4dswyQNBfohGpz9o6Yb3Gz3SrUDqBH5rTuhGR7ahtlbYKnVxw2bCFMRljaA7EXHaXZ8wsHdodFvbkhKmqg==";
      key = "concat-map@0.0.1";
      name = "concat-map";
      resolved = "https://registry.npmjs.org/concat-map/-/concat-map-0.0.1.tgz";
      version = "0.0.1";
    };
    "diff-sequences@28.1.1" = {
      dev = true;
      integrity =
        "sha512-FU0iFaH/E23a+a718l8Qa/19bF9p06kgE0KipMOMadwa3SjnaElKzPaUC0vnibs6/B/9ni97s61mcejk8W1fQw==";
      key = "diff-sequences@28.1.1";
      name = "diff-sequences";
      resolved =
        "https://registry.npmjs.org/diff-sequences/-/diff-sequences-28.1.1.tgz";
      version = "28.1.1";
    };
    "fs-extra@8.1.0" = {
      dev = true;
      integrity =
        "sha512-yhlQgA6mnOJUKOsRUFsgJdQCvkKhcz8tlZG5HBQfReYZy46OwLcY+Zia0mtdHsOo9y/hP+CxMN0TU9QxoOtG4g==";
      key = "fs-extra@8.1.0";
      name = "fs-extra";
      requires = {
        graceful-fs = "^4.2.0";
        jsonfile = "^4.0.0";
        universalify = "^0.1.0";
      };
      resolved = "https://registry.npmjs.org/fs-extra/-/fs-extra-8.1.0.tgz";
      version = "8.1.0";
    };
    "fs.realpath@1.0.0" = {
      dev = true;
      integrity =
        "sha512-OO0pH2lK6a0hZnAdau5ItzHPI6pUlvI7jMVnxUQRtw4owF2wk8lOSabtGDCTP4Ggrg2MbGnWO9X8K1t4+fGMDw==";
      key = "fs.realpath@1.0.0";
      name = "fs.realpath";
      resolved =
        "https://registry.npmjs.org/fs.realpath/-/fs.realpath-1.0.0.tgz";
      version = "1.0.0";
    };
    "function-bind@1.1.1" = {
      dev = true;
      integrity =
        "sha512-yIovAzMX49sF8Yl58fSCWJ5svSLuaibPxXQJFLmBObTuCr0Mf1KiPopGM9NiFjiYBCbfaa2Fh6breQ6ANVTI0A==";
      key = "function-bind@1.1.1";
      name = "function-bind";
      resolved =
        "https://registry.npmjs.org/function-bind/-/function-bind-1.1.1.tgz";
      version = "1.1.1";
    };
    "glob@7.2.3" = {
      dev = true;
      integrity =
        "sha512-nFR0zLpU2YCaRxwoCJvL6UvCH2JFyFVIvwTLsIf21AuHlMskA1hhTdk+LlYJtOlYt9v6dvszD2BGRqBL+iQK9Q==";
      key = "glob@7.2.3";
      name = "glob";
      requires = {
        "fs.realpath" = "^1.0.0";
        inflight = "^1.0.4";
        inherits = "2";
        minimatch = "^3.1.1";
        once = "^1.3.0";
        path-is-absolute = "^1.0.0";
      };
      resolved = "https://registry.npmjs.org/glob/-/glob-7.2.3.tgz";
      version = "7.2.3";
    };
    "graceful-fs@4.2.10" = {
      dev = true;
      integrity =
        "sha512-9ByhssR2fPVsNZj478qUUbKfmL0+t5BDVyjShtyZZLiK7ZDAArFFfopyOTj0M05wE2tJPisA4iTnnXl2YoPvOA==";
      key = "graceful-fs@4.2.10";
      name = "graceful-fs";
      resolved =
        "https://registry.npmjs.org/graceful-fs/-/graceful-fs-4.2.10.tgz";
      version = "4.2.10";
    };
    "handlebars@4.7.7" = {
      dev = true;
      integrity =
        "sha512-aAcXm5OAfE/8IXkcZvCepKU3VzW1/39Fb5ZuqMtgI/hT8X2YgoMvBY5dLhq/cpOvw7Lk1nK/UF71aLG/ZnVYRA==";
      key = "handlebars@4.7.7";
      name = "handlebars";
      requires = {
        minimist = "^1.2.5";
        neo-async = "^2.6.0";
        source-map = "^0.6.1";
        uglify-js = "^3.1.4";
        wordwrap = "^1.0.0";
      };
      resolved = "https://registry.npmjs.org/handlebars/-/handlebars-4.7.7.tgz";
      version = "4.7.7";
    };
    "has-flag@4.0.0" = {
      dev = true;
      integrity =
        "sha512-EykJT/Q1KjTWctppgIAgfSO0tKVuZUjhgMr17kqTumMl6Afv3EISleU7qZUzoXDFTAHTDC4NOoG/ZxU3EvlMPQ==";
      key = "has-flag@4.0.0";
      name = "has-flag";
      resolved = "https://registry.npmjs.org/has-flag/-/has-flag-4.0.0.tgz";
      version = "4.0.0";
    };
    "has@1.0.3" = {
      dev = true;
      integrity =
        "sha512-f2dvO0VU6Oej7RkWJGrehjbzMAjFp5/VKPp5tTpWIV4JHHZK1/BxbFRtf/siA2SWTe09caDmVtYYzWEIbBS4zw==";
      key = "has@1.0.3";
      name = "has";
      requires = { function-bind = "^1.1.1"; };
      resolved = "https://registry.npmjs.org/has/-/has-1.0.3.tgz";
      version = "1.0.3";
    };
    "highlight.js@9.18.5" = {
      dev = true;
      integrity =
        "sha512-a5bFyofd/BHCX52/8i8uJkjr9DYwXIPnM/plwI6W7ezItLGqzt7X2G2nXuYSfsIJdkwwj/g9DG1LkcGJI/dDoA==";
      key = "highlight.js@9.18.5";
      name = "highlight.js";
      resolved =
        "https://registry.npmjs.org/highlight.js/-/highlight.js-9.18.5.tgz";
      version = "9.18.5";
    };
    "inflight@1.0.6" = {
      dev = true;
      integrity =
        "sha512-k92I/b08q4wvFscXCLvqfsHCrjrF7yiXsQuIVvVE7N82W3+aqpzuUdBbfhWcy/FZR3/4IgflMgKLOsvPDrGCJA==";
      key = "inflight@1.0.6";
      name = "inflight";
      requires = {
        once = "^1.3.0";
        wrappy = "1";
      };
      resolved = "https://registry.npmjs.org/inflight/-/inflight-1.0.6.tgz";
      version = "1.0.6";
    };
    "inherits@2.0.4" = {
      dev = true;
      integrity =
        "sha512-k/vGaX4/Yla3WzyMCvTQOXYeIHvqOKtnqBduzTHpzpQZzAskKMhZ2K+EnBiSM9zGSoIFeMpXKxa4dYeZIQqewQ==";
      key = "inherits@2.0.4";
      name = "inherits";
      resolved = "https://registry.npmjs.org/inherits/-/inherits-2.0.4.tgz";
      version = "2.0.4";
    };
    "interpret@1.4.0" = {
      dev = true;
      integrity =
        "sha512-agE4QfB2Lkp9uICn7BAqoscw4SZP9kTE2hxiFI3jBPmXJfdqiahTbUuKGsMoN2GtqL9AxhYioAcVvgsb1HvRbA==";
      key = "interpret@1.4.0";
      name = "interpret";
      resolved = "https://registry.npmjs.org/interpret/-/interpret-1.4.0.tgz";
      version = "1.4.0";
    };
    "is-core-module@2.9.0" = {
      dev = true;
      integrity =
        "sha512-+5FPy5PnwmO3lvfMb0AsoPaBG+5KHUI0wYFXOtYPnVVVspTFUuMZNfNaNVRt3FZadstu2c8x23vykRW/NBoU6A==";
      key = "is-core-module@2.9.0";
      name = "is-core-module";
      requires = { has = "^1.0.3"; };
      resolved =
        "https://registry.npmjs.org/is-core-module/-/is-core-module-2.9.0.tgz";
      version = "2.9.0";
    };
    "jest-diff@28.1.1" = {
      dev = true;
      integrity =
        "sha512-/MUUxeR2fHbqHoMMiffe/Afm+U8U4olFRJ0hiVG2lZatPJcnGxx292ustVu7bULhjV65IYMxRdploAKLbcrsyg==";
      key = "jest-diff@28.1.1";
      name = "jest-diff";
      requires = {
        chalk = "^4.0.0";
        diff-sequences = "^28.1.1";
        jest-get-type = "^28.0.2";
        pretty-format = "^28.1.1";
      };
      resolved = "https://registry.npmjs.org/jest-diff/-/jest-diff-28.1.1.tgz";
      version = "28.1.1";
    };
    "jest-get-type@28.0.2" = {
      dev = true;
      integrity =
        "sha512-ioj2w9/DxSYHfOm5lJKCdcAmPJzQXmbM/Url3rhlghrPvT3tt+7a/+oXc9azkKmLvoiXjtV83bEWqi+vs5nlPA==";
      key = "jest-get-type@28.0.2";
      name = "jest-get-type";
      resolved =
        "https://registry.npmjs.org/jest-get-type/-/jest-get-type-28.0.2.tgz";
      version = "28.0.2";
    };
    "jquery@3.6.0" = {
      dev = true;
      integrity =
        "sha512-JVzAR/AjBvVt2BmYhxRCSYysDsPcssdmTFnzyLEts9qNwmjmu4JTAMYubEfwVOSwpQ1I1sKKFcxhZCI2buerfw==";
      key = "jquery@3.6.0";
      name = "jquery";
      resolved = "https://registry.npmjs.org/jquery/-/jquery-3.6.0.tgz";
      version = "3.6.0";
    };
    "jsonfile@4.0.0" = {
      dev = true;
      integrity =
        "sha512-m6F1R3z8jjlf2imQHS2Qez5sjKWQzbuuhuJ/FKYFRZvPE3PuHcSMVZzfsLhGVOkfd20obL5SWEBew5ShlquNxg==";
      key = "jsonfile@4.0.0";
      name = "jsonfile";
      requires = { graceful-fs = "^4.1.6"; };
      resolved = "https://registry.npmjs.org/jsonfile/-/jsonfile-4.0.0.tgz";
      version = "4.0.0";
    };
    "lodash@4.17.21" = {
      dev = true;
      integrity =
        "sha512-v2kDEe57lecTulaDIuNTPy3Ry4gLGJ6Z1O3vE1krgXZNrsQ+LFTGHVxVjcXPs17LhbZVGedAJv8XZ1tvj5FvSg==";
      key = "lodash@4.17.21";
      name = "lodash";
      resolved = "https://registry.npmjs.org/lodash/-/lodash-4.17.21.tgz";
      version = "4.17.21";
    };
    "lunr@2.3.9" = {
      dev = true;
      integrity =
        "sha512-zTU3DaZaF3Rt9rhN3uBMGQD3dD2/vFQqnvZCDv4dl5iOzq2IZQqTxu90r4E5J+nP70J3ilqVCrbho2eWaeW8Ow==";
      key = "lunr@2.3.9";
      name = "lunr";
      resolved = "https://registry.npmjs.org/lunr/-/lunr-2.3.9.tgz";
      version = "2.3.9";
    };
    "marked@0.7.0" = {
      dev = true;
      integrity =
        "sha512-c+yYdCZJQrsRjTPhUx7VKkApw9bwDkNbHUKo1ovgcfDjb2kc8rLuRbIFyXL5WOEUwzSSKo3IXpph2K6DqB/KZg==";
      key = "marked@0.7.0";
      name = "marked";
      resolved = "https://registry.npmjs.org/marked/-/marked-0.7.0.tgz";
      version = "0.7.0";
    };
    "minimatch@3.1.2" = {
      dev = true;
      integrity =
        "sha512-J7p63hRiAjw1NDEww1W7i37+ByIrOWO5XQQAzZ3VOcL0PNybwpfmV/N05zFAzwQ9USyEcX6t3UO+K5aqBQOIHw==";
      key = "minimatch@3.1.2";
      name = "minimatch";
      requires = { brace-expansion = "^1.1.7"; };
      resolved = "https://registry.npmjs.org/minimatch/-/minimatch-3.1.2.tgz";
      version = "3.1.2";
    };
    "minimist@1.2.6" = {
      dev = true;
      integrity =
        "sha512-Jsjnk4bw3YJqYzbdyBiNsPWHPfO++UGG749Cxs6peCu5Xg4nrena6OVxOYxrQTqww0Jmwt+Ref8rggumkTLz9Q==";
      key = "minimist@1.2.6";
      name = "minimist";
      resolved = "https://registry.npmjs.org/minimist/-/minimist-1.2.6.tgz";
      version = "1.2.6";
    };
    "nan@2.14.2" = {
      integrity =
        "sha512-M2ufzIiINKCuDfBSAUr1vWQ+vuVcA9kqx8JJUsbQi6yf1uGRyb7HfpdfUr5qLXf3B/t8dPvcjhKMmlfnP47EzQ==";
      key = "nan@2.14.2";
      name = "nan";
      resolved = "https://registry.npmjs.org/nan/-/nan-2.14.2.tgz";
      version = "2.14.2";
    };
    "neo-async@2.6.2" = {
      dev = true;
      integrity =
        "sha512-Yd3UES5mWCSqR+qNT93S3UoYUkqAZ9lLg8a7g9rimsWmYGK8cVToA4/sF3RrshdyV3sAGMXVUmpMYOw+dLpOuw==";
      key = "neo-async@2.6.2";
      name = "neo-async";
      resolved = "https://registry.npmjs.org/neo-async/-/neo-async-2.6.2.tgz";
      version = "2.6.2";
    };
    "node-gyp-build@4.4.0" = {
      integrity =
        "sha512-amJnQCcgtRVw9SvoebO3BKGESClrfXGCUTX9hSn1OuGQTQBOZmVd0Z0OlecpuRksKvbsUqALE8jls/ErClAPuQ==";
      key = "node-gyp-build@4.4.0";
      name = "node-gyp-build";
      resolved =
        "https://registry.npmjs.org/node-gyp-build/-/node-gyp-build-4.4.0.tgz";
      version = "4.4.0";
    };
    "once@1.4.0" = {
      dev = true;
      integrity =
        "sha512-lNaJgI+2Q5URQBkccEKHTQOPaXdUxnZZElQTZY0MFUAuaEqe1E+Nyvgdz/aIyNi6Z9MzO5dv1H8n58/GELp3+w==";
      key = "once@1.4.0";
      name = "once";
      requires = { wrappy = "1"; };
      resolved = "https://registry.npmjs.org/once/-/once-1.4.0.tgz";
      version = "1.4.0";
    };
    "path-is-absolute@1.0.1" = {
      dev = true;
      integrity =
        "sha512-AVbw3UJ2e9bq64vSaS9Am0fje1Pa8pbGqTTsmXfaIiMpnr5DlDhfJOuLj9Sf95ZPVDAUerDfEk88MPmPe7UCQg==";
      key = "path-is-absolute@1.0.1";
      name = "path-is-absolute";
      resolved =
        "https://registry.npmjs.org/path-is-absolute/-/path-is-absolute-1.0.1.tgz";
      version = "1.0.1";
    };
    "path-parse@1.0.7" = {
      dev = true;
      integrity =
        "sha512-LDJzPVEEEPR+y48z93A0Ed0yXb8pAByGWo/k5YYdYgpY2/2EsOsksJrq7lOHxryrVOn1ejG6oAp8ahvOIQD8sw==";
      key = "path-parse@1.0.7";
      name = "path-parse";
      resolved = "https://registry.npmjs.org/path-parse/-/path-parse-1.0.7.tgz";
      version = "1.0.7";
    };
    "pretty-format@28.1.1" = {
      dependencies = {
        ansi-styles = {
          dev = true;
          integrity =
            "sha512-Cxwpt2SfTzTtXcfOlzGEee8O+c+MmUgGrNiBcXnuWxuFJHe6a5Hz7qwhwe5OgaSYI0IJvkLqWX1ASG+cJOkEiA==";
          resolved =
            "https://registry.npmjs.org/ansi-styles/-/ansi-styles-5.2.0.tgz";
          version = "5.2.0";
        };
      };
      dev = true;
      integrity =
        "sha512-wwJbVTGFHeucr5Jw2bQ9P+VYHyLdAqedFLEkdQUVaBF/eiidDwH5OpilINq4mEfhbCjLnirt6HTTDhv1HaTIQw==";
      key = "pretty-format@28.1.1";
      name = "pretty-format";
      requires = {
        "@jest/schemas" = "^28.0.2";
        ansi-regex = "^5.0.1";
        ansi-styles = "^5.0.0";
        react-is = "^18.0.0";
      };
      resolved =
        "https://registry.npmjs.org/pretty-format/-/pretty-format-28.1.1.tgz";
      version = "28.1.1";
    };
    "progress@2.0.3" = {
      dev = true;
      integrity =
        "sha512-7PiHtLll5LdnKIMw100I+8xJXR5gW2QwWYkT6iJva0bXitZKa/XMrSbdmg3r2Xnaidz9Qumd0VPaMrZlF9V9sA==";
      key = "progress@2.0.3";
      name = "progress";
      resolved = "https://registry.npmjs.org/progress/-/progress-2.0.3.tgz";
      version = "2.0.3";
    };
    "react-is@18.1.0" = {
      dev = true;
      integrity =
        "sha512-Fl7FuabXsJnV5Q1qIOQwx/sagGF18kogb4gpfcG4gjLBWO0WDiiz1ko/ExayuxE7InyQkBLkxRFG5oxY6Uu3Kg==";
      key = "react-is@18.1.0";
      name = "react-is";
      resolved = "https://registry.npmjs.org/react-is/-/react-is-18.1.0.tgz";
      version = "18.1.0";
    };
    "rechoir@0.6.2" = {
      dev = true;
      integrity =
        "sha512-HFM8rkZ+i3zrV+4LQjwQ0W+ez98pApMGM3HUrN04j3CqzPOzl9nmP15Y8YXNm8QHGv/eacOVEjqhmWpkRV0NAw==";
      key = "rechoir@0.6.2";
      name = "rechoir";
      requires = { resolve = "^1.1.6"; };
      resolved = "https://registry.npmjs.org/rechoir/-/rechoir-0.6.2.tgz";
      version = "0.6.2";
    };
    "resolve@1.22.0" = {
      dev = true;
      integrity =
        "sha512-Hhtrw0nLeSrFQ7phPp4OOcVjLPIeMnRlr5mcnVuMe7M/7eBn98A3hmFRLoFo3DLZkivSYwhRUJTyPyWAk56WLw==";
      key = "resolve@1.22.0";
      name = "resolve";
      requires = {
        is-core-module = "^2.8.1";
        path-parse = "^1.0.7";
        supports-preserve-symlinks-flag = "^1.0.0";
      };
      resolved = "https://registry.npmjs.org/resolve/-/resolve-1.22.0.tgz";
      version = "1.22.0";
    };
    "serialize-error@5.0.0" = {
      integrity =
        "sha512-/VtpuyzYf82mHYTtI4QKtwHa79vAdU5OQpNPAmE/0UDdlGT0ZxHwC+J6gXkw29wwoVI8fMPsfcVHOwXtUQYYQA==";
      key = "serialize-error@5.0.0";
      name = "serialize-error";
      requires = { type-fest = "^0.8.0"; };
      resolved =
        "https://registry.npmjs.org/serialize-error/-/serialize-error-5.0.0.tgz";
      version = "5.0.0";
    };
    "shelljs@0.8.5" = {
      dev = true;
      integrity =
        "sha512-TiwcRcrkhHvbrZbnRcFYMLl30Dfov3HKqzp5tO5b4pt6G/SezKcYhmDg15zXVBswHmctSAQKznqNW2LO5tTDow==";
      key = "shelljs@0.8.5";
      name = "shelljs";
      requires = {
        glob = "^7.0.0";
        interpret = "^1.0.0";
        rechoir = "^0.6.2";
      };
      resolved = "https://registry.npmjs.org/shelljs/-/shelljs-0.8.5.tgz";
      version = "0.8.5";
    };
    "source-map@0.6.1" = {
      dev = true;
      integrity =
        "sha512-UjgapumWlbMhkBgzT7Ykc5YXUT46F0iKu8SGXq0bcwP5dz/h0Plj6enJqjz1Zbq2l5WaqYnrVbwWOWMyF3F47g==";
      key = "source-map@0.6.1";
      name = "source-map";
      resolved = "https://registry.npmjs.org/source-map/-/source-map-0.6.1.tgz";
      version = "0.6.1";
    };
    "supports-color@7.2.0" = {
      dev = true;
      integrity =
        "sha512-qpCAvRl9stuOHveKsn7HncJRvv501qIacKzQlO/+Lwxc9+0q2wLyv4Dfvt80/DPn2pqOBsJdDiogXGR9+OvwRw==";
      key = "supports-color@7.2.0";
      name = "supports-color";
      requires = { has-flag = "^4.0.0"; };
      resolved =
        "https://registry.npmjs.org/supports-color/-/supports-color-7.2.0.tgz";
      version = "7.2.0";
    };
    "supports-preserve-symlinks-flag@1.0.0" = {
      dev = true;
      integrity =
        "sha512-ot0WnXS9fgdkgIcePe6RHNk1WA8+muPa6cSjeR3V8K27q9BB1rTE3R1p7Hv0z1ZyAc8s6Vvv8DIyWf681MAt0w==";
      key = "supports-preserve-symlinks-flag@1.0.0";
      name = "supports-preserve-symlinks-flag";
      resolved =
        "https://registry.npmjs.org/supports-preserve-symlinks-flag/-/supports-preserve-symlinks-flag-1.0.0.tgz";
      version = "1.0.0";
    };
    "type-fest@0.8.1" = {
      integrity =
        "sha512-4dbzIzqvjtgiM5rw1k5rEHtBANKmdudhGyBEajN01fEyhaAIhsoKNy6y7+IN93IfpFtwY9iqi7kD+xwKhQsNJA==";
      key = "type-fest@0.8.1";
      name = "type-fest";
      resolved = "https://registry.npmjs.org/type-fest/-/type-fest-0.8.1.tgz";
      version = "0.8.1";
    };
    "typedoc-default-themes@0.6.3" = {
      dev = true;
      integrity =
        "sha512-rouf0TcIA4M2nOQFfC7Zp4NEwoYiEX4vX/ZtudJWU9IHA29MPC+PPgSXYLPESkUo7FuB//GxigO3mk9Qe1xp3Q==";
      key = "typedoc-default-themes@0.6.3";
      name = "typedoc-default-themes";
      requires = {
        backbone = "^1.4.0";
        jquery = "^3.4.1";
        lunr = "^2.3.8";
        underscore = "^1.9.1";
      };
      resolved =
        "https://registry.npmjs.org/typedoc-default-themes/-/typedoc-default-themes-0.6.3.tgz";
      version = "0.6.3";
    };
    "typedoc@0.15.5" = {
      dependencies = {
        typescript = {
          dev = true;
          integrity =
            "sha512-MmQdgo/XenfZPvVLtKZOq9jQQvzaUAUpcKW8Z43x9B2fOm4S5g//tPtMweZUIP+SoBqrVPEIm+dJeQ9dfO0QdA==";
          resolved =
            "https://registry.npmjs.org/typescript/-/typescript-3.7.7.tgz";
          version = "3.7.7";
        };
      };
      dev = true;
      integrity =
        "sha512-AKXLtOUCLRlSTyfXQHYp3LFPy6RiFLnxnKS5z1jwQsYXmCPbHWuhmfgS264Es2hPMZjzvHqk/ZQDzCBpb49u6w==";
      key = "typedoc@0.15.5";
      name = "typedoc";
      requires = {
        "@types/minimatch" = "3.0.3";
        fs-extra = "^8.1.0";
        handlebars = "^4.5.3";
        "highlight.js" = "^9.17.1";
        lodash = "^4.17.15";
        marked = "^0.7.0";
        minimatch = "^3.0.0";
        progress = "^2.0.3";
        shelljs = "^0.8.3";
        typedoc-default-themes = "^0.6.2";
        typescript = "3.7.x";
      };
      resolved = "https://registry.npmjs.org/typedoc/-/typedoc-0.15.5.tgz";
      version = "0.15.5";
    };
    "typescript@3.7.7" = {
      dev = true;
      integrity =
        "sha512-MmQdgo/XenfZPvVLtKZOq9jQQvzaUAUpcKW8Z43x9B2fOm4S5g//tPtMweZUIP+SoBqrVPEIm+dJeQ9dfO0QdA==";
      key = "typescript@3.7.7";
      name = "typescript";
      resolved = "https://registry.npmjs.org/typescript/-/typescript-3.7.7.tgz";
      version = "3.7.7";
    };
    "typescript@3.9.5" = {
      dev = true;
      integrity =
        "sha512-hSAifV3k+i6lEoCJ2k6R2Z/rp/H3+8sdmcn5NrS3/3kE7+RyZXm9aqvxWqjEXHAd8b0pShatpcdMTvEdvAJltQ==";
      key = "typescript@3.9.5";
      name = "typescript";
      resolved = "https://registry.npmjs.org/typescript/-/typescript-3.9.5.tgz";
      version = "3.9.5";
    };
    "uglify-js@3.16.0" = {
      dev = true;
      integrity =
        "sha512-FEikl6bR30n0T3amyBh3LoiBdqHRy/f4H80+My34HOesOKyHfOsxAPAxOoqC0JUnC1amnO0IwkYC3sko51caSw==";
      key = "uglify-js@3.16.0";
      name = "uglify-js";
      optional = true;
      resolved = "https://registry.npmjs.org/uglify-js/-/uglify-js-3.16.0.tgz";
      version = "3.16.0";
    };
    "underscore@1.13.4" = {
      dev = true;
      integrity =
        "sha512-BQFnUDuAQ4Yf/cYY5LNrK9NCJFKriaRbD9uR1fTeXnBeoa97W0i41qkZfGO9pSo8I5KzjAcSY2XYtdf0oKd7KQ==";
      key = "underscore@1.13.4";
      name = "underscore";
      resolved =
        "https://registry.npmjs.org/underscore/-/underscore-1.13.4.tgz";
      version = "1.13.4";
    };
    "universalify@0.1.2" = {
      dev = true;
      integrity =
        "sha512-rBJeI5CXAlmy1pV+617WB9J63U6XcazHHF2f2dbJix4XzpUF0RS3Zbj0FGIOCAva5P/d/GBOYaACQ1w+0azUkg==";
      key = "universalify@0.1.2";
      name = "universalify";
      resolved =
        "https://registry.npmjs.org/universalify/-/universalify-0.1.2.tgz";
      version = "0.1.2";
    };
    "wordwrap@1.0.0" = {
      dev = true;
      integrity = "sha1-J1hIEIkUVqQXHI0CJkQa3pDLyus=";
      key = "wordwrap@1.0.0";
      name = "wordwrap";
      resolved = "https://registry.npmjs.org/wordwrap/-/wordwrap-1.0.0.tgz";
      version = "1.0.0";
    };
    "wrappy@1.0.2" = {
      dev = true;
      integrity = "sha1-tSQ9jz7BqjXxNkYFvA0QNuMKtp8=";
      key = "wrappy@1.0.2";
      name = "wrappy";
      resolved = "https://registry.npmjs.org/wrappy/-/wrappy-1.0.2.tgz";
      version = "1.0.2";
    };
    "zeromq@5.2.8" = {
      integrity =
        "sha512-bXzsk7KOmgLSv1tC0Ms1VXBy90+Rz27ZYf27cLuldRYbpqYpuWJfxxHFhO710t22zgWBnmdUP0m3SKFpLI0u5g==";
      key = "zeromq@5.2.8";
      name = "zeromq";
      requires = {
        nan = "2.14.2";
        node-gyp-build = "^4.2.3";
      };
      resolved = "https://registry.npmjs.org/zeromq/-/zeromq-5.2.8.tgz";
      version = "5.2.8";
    };
  };
  wrong = { };
}
