...
** Examples
The framework can be daunting, there's a lot it can do
but it might be hard to know where to start.

Personally I think a good jumping off point is reading
how we bootstrap [[pacote][./pkgs/tools/pacote/pacote.nix]]
...
first project using [[simple template][./templates/simple]]
...
[[msgpack][./tests/build-support/msgpack.nix]].

There's similar simple projects out on the =flocoPackages= repo
especially under the ~pkgs/BINS/*~ directory.

Past that read the =README.org= files under ~pkgs/~, and then
explore the test suite examples and more of =flocoPackages=. 
