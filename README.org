...
At time of writing there =lib.libmeta.metaSetFromSerial= has not been implemented, so you'll need to manually call =lib.libmeta.metaEntFromSerial= and =lib.libmeta.mkMetaSet= to deserialize. This is going to be implemented ASAP. Honestly it's a simple process and I could have written it in the time ittook to write this note. 
